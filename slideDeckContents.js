...
responseDiv.innerHTML = "<img src=\"spinner.gif\" height=\"120px\" width=\"120px\" \>";
    ...
    const numberSlides = document.getElementById('numberSlides').value;

    try {
      // Make a POST request to the API
      const response = await fetch('http://localhost:3000/api/slideDeckContents', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ aimedAt, whoWant, butFear, numberSlides }),
      });
      ...
      if (response.ok) {
         ...
         responseDiv.innerHTML = "<p>" + result.choices[0].message.content + "</p>";
